...
def add_dust(dust, img, alpha, beta):
    ...
    # 马赛克、模糊
    # dust = corrupt(dust, corruption_name='jpeg_compression', severity=2)

    # 对生成噪线图进行鱼眼效果的改变
    # dust = transform_dust(dust, ratio=np.random.uniform(1, 10))
...
if __name__ == '__main__':
    image_path = r'D:\Dataset\data\genrain\dustSource'
    image_out_path = r'D:\Dataset\data\genrain\dustNew'
    ...
    for l in list:
        ...
        dust = dust_blur(noise, noise2, noise3, length=length_random, length2=101, length3=80, angle1=angle_random1,
                         angle2=angle_random2, angle3=angle_random3, w=3, w2=3, w3=3)  # angle取[-25,25]
        # 图片融合
        ...
        # 随机噪声
        noise_random = np.random.uniform(0.8, 0.9)
        noise = get_noise(img, value=noise_random)
        noise2 = get_noise(img, value=noise_random)
        noise3 = get_noise(img, value=noise_random)
        # 噪声调整
