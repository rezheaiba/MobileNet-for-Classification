...
def main():
    ...
    data_root = r'D:\Dataset\data'  # get data root path D:\Dataset\data\level1_aug_splited
    image_path = os.path.join(data_root, "level1-450_plus_plus_splited")  # flower data set path
    ...
    with open('level1_plus_plus_class_indices.json', 'w') as json_file:
    ...
    # net = MobileNetV2(7)
    # model_weight_path = 'weight-1-plus/MobileNetV2_level1_best.pth'
    ...
    # loss_function = LabelSmoothing(0.1).to(device)
    ...
    # best_acc = 0.0
    best_acc = 0.982
    save_path_best = 'weight-1-plus-plus/MobileNetV2_level1_best.pth'
    save_path_newest = 'weight-1-plus-plus/MobileNetV2_level1_newest.pth'
    train_steps = len(train_loader)
    val_steps = len(validate_loader)
    prev_time = time.time()

    if epochi != 0:
        net.load_state_dict(torch.load(save_path_best, map_location=device))
