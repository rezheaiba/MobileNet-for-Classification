...
class MobileNetV3(nn.Module):
    def __init__(
        ...
        self.classifier3 = nn.Sequential(
            ...
            nn.ReLU6(inplace=True),
            nn.Dropout(p=dropout, inplace=True),
            nn.Linear(last_channel, num_classes_3)
        )
        self.classifier4 = nn.Sequential(
            nn.Linear(lastconv_output_channel, last_channel),
            nn.ReLU6(inplace=True),
            nn.Dropout(p=dropout, inplace=True),
            nn.Linear(last_channel, num_classes_4)
        )
    ...
    def forward(self, x: Tensor) -> dict:
        ...
        return {
            ...
            'classifier4': self.classifier4(x),
        }


def main():
    input = torch.randn([8, 3, 256, 256])
    model = MobileNetV3(dilated=True, num_classes_1=2, num_classes_2=5, num_classes_3=2, num_classes_4=2, arch='mobilenet_v3_small',
                        reduced_tail=True, width_mult=0.5).cpu()
    model.eval()
    input = input.cpu()
    out = model(input)
    print(out['classifier1'].shape)
    print(out['classifier2'].shape)
    torch.save(model.state_dict(), 'model_v3_mult.pth')
